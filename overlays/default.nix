...
final: prev: {
  ...
  # Windows_11_dark cursor — your uploaded .cur/.ani set converted to XCursor.
  # win2xcur turns each Windows cursor into a Linux X cursor; we lay them out as
  # a proper icon theme so Stylix/Plasma can use it system-wide.
  win11-dark-cursors = prev.stdenv.mkDerivation {
    ...
    buildPhase = ''
      runHook preBuild
      mkdir -p out
      # Convert every Windows cursor (.cur/.ani) to XCursor format.
    ...
    installPhase = ''
      ...
      # index.theme so it shows up as a selectable cursor theme.
      ...
      # Map common XCursor names to the converted files so apps that request
      # standard names (left_ptr, text, watch, etc.) resolve correctly.
      cd "$THEME/cursors"
      link() { [ -e "$2" ] && ln -sf "$2" "$1" || true; }
      link left_ptr      pointer
      link default       pointer
      link text          beam
      link xterm         beam
      link watch         busy
      link progress      working
      link hand2         link
      link pointer       link
      link help          help
      link move          move
      ...
      link not-allowed   unavailable
      link no-drop       unavailable
      runHook postInstall
    '';
